...
<div class="section relative mb-10 flex h-fit min-h-screen flex-col gap-2 px-0 md:h-screen md:flex-row md:px-10" id="section-stats">
    ...
    <Stat id="ratingIncrease" number="95%" name="average increase in rating" color="#FFD700">
        {#each Array(4) as _value, quadrant}
            <div class="absolute h-[calc(50%-8%)] w-[calc(50%-8%)]" style="top: calc({25 * (quadrant == 0 || quadrant == 1 ? 0 : 2)}% + {8 * (quadrant == 0 || quadrant == 1 ? 0 : 1)}%); left: calc({25 * (quadrant == 0 || quadrant == 2 ? 0 : 2)}% + {8 * (quadrant == 0 || quadrant == 2 ? 0 : 1)}%);">
